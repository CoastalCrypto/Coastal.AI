// Copies the flat folder-bundled sidecar (runtime binary + app/) from
...
//
// The app/ tree is a flat npm install (see core/scripts/bundle-sidecar.mjs):
// real files, no .pnpm store, no junctions — so a plain recursive copy is safe.
...
// 2. Copy the flat app/ (dist + node_modules) into resources/app.
rmSync(resolve(resDir, 'app'), { recursive: true, force: true })
mkdirSync(resDir, { recursive: true })
cpSync(resolve(srcBuild, 'app'), resolve(resDir, 'app'), { recursive: true })

console.log('[sync-sidecar] staged', runtimes.join(', '), '+ flat app/ into src-tauri/resources/')
