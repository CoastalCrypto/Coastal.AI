...
- [~] **Syncthing** notes-substrate replication — **library complete + verified** (spec `docs/superpowers/specs/2026-06-18-syncthing-replication-design.md`, plan `…/plans/2026-06-18-syncthing-replication.md`): Lamport-rev `NoteStore`, frontmatter codec, exporter/ingester, e2e bridge test, Ed25519-gated Syncthing REST provisioning, per-role bridge driver, `os/base/systemd/coastal-syncthing.service`, `syncthing` in both OS package lists. **Wiring building blocks now in place + exported** (all tested): peer-registry `setDeviceId`/`getDeviceId`/`knownDeviceIds`, `buildWorkerFolders`/`buildCuratorFolders` (hub-and-spoke `DesiredFolder[]`), `reconcileSyncthing`. **Cluster-gated remainder** (needs first-boot cluster-join config + role detection): at join, `setDeviceId` for each peer then `reconcileSyncthing(http, { …, knownDeviceIds: registry.knownDeviceIds() })`; schedule `runWorkerTick`/`runCuratorTick` by role; wire the Curator `keep` hook to `createCuratorDaemon`; manual 2-container E2E.
- [~] **openobserve** Monitor/Watchdog backend — **library complete + verified** (spec `…/specs/2026-06-19-openobserve-monitor-design.md`, plan `…/plans/2026-06-19-openobserve-monitor.md`): typed `openobserve-client` (ingest/query over injected fetch), pure `evaluateHealth` (heartbeats→alerts), `os/base/systemd/coastal-openobserve.service`. **Wiring building blocks now in place + exported** (tested): `checkClusterHealth(client, expected, now, staleness)` composes query→`evaluateHealth`→alerts. **Cluster-gated remainder**: emit per-node heartbeats (ingest `heartbeats` stream), the daemon interval that calls `checkClusterHealth`→notify, and fetching the openobserve binary into the image (not an apt package). Zombie reclaim already exists in `coordination/transitions/reclaim.ts`.
